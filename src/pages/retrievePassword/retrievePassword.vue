      ...
      <div class="middle-wrapper-right">
        ...
        <div class="m-right-content">
          ...
          <el-form class="p-form reset-form-input reset-form-required" :model="passwordForm" :rules="rules"
                   ref="passwordForm">
            <el-form-item label="邮箱/手机号" prop="password">
              <el-input placeholder="请输入邮箱或手机号" v-model="passwordForm.phoneCode"></el-input>
            </el-form-item>
          </el-form>
          <div class="submit-line">
            <el-button class="submit-btn" type="primary" @click="submitForm('passwordForm')">发送验证码</el-button>
          </div>
        </div>
      </div>
    </div>
    ...
    export default {
        data() {
            return {
                ...
                rules: {
                    phoneCode: [{
                        required: true, message: '请输入邮箱或手机号不能为空', trigger: 'blur'
                    },
                    ]
                }
            }
        ...
        methods: {
            ...mapActions(['getFindPwdCodeRequest']),
            submitForm(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        if (!checkTel(this.passwordForm.phoneCode) && !isEmail(this.passwordForm.phoneCode)) {
                            this.$message({
                        ...
                        this.getFindPwdCodeRequest({
                            username: this.passwordForm.phoneCode
                        })
                            .then(res => {
                                if (res.code == '200') {
                                    this.$message({
